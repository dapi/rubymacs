...
(defun mc--shell-compile-without-asking-for-args (shell dbgflags &optional errors-regexp-alist)
  ;; Run SHELL with debug flags DBGFLAGS on current-buffer
  (let* ((shcmd   (or (mc--which shell)
                      (error "Compilation abort: command %s not found" shell)))
         (shfile  (or mc--remote-pathname (buffer-file-name)
...
User is NOT prompted for arguments to run their ruby program with.
...
See variables compilation-error-regexp-alist or ruby-compilation-error-regexp-alist."
  (mc--shell-compile-without-asking-for-args ruby-command ruby-dbg-flags ruby-compilation-error-regexp-alist))
